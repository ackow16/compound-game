        ...
        try {
            ...
            if (isPhone) {
                ...
                if (WORD_COUNT === 12) {
                    ...
                    // Increased max ratio to 3.5 for even taller rings
                    aspectRatio = Math.max(2.5, Math.min(3.5, 3.0 - (screenRatio - 0.4) * 0.3));
                } else {
                    // 16-word phone: 3x7 layout - target ~2.33 (7/3)
                    if (screenRatio < 0.4) {
                        aspectRatio = 2.4 + (0.4 - screenRatio) * 0.6;
                    } else {
                        aspectRatio = 2.6 - (screenRatio - 0.4) * 0.3;
                    }
                    // Increased max from 3.0 to 3.4 for maximum vertical space
                    aspectRatio = Math.max(2.2, Math.min(3.4, aspectRatio));
                }
            } else {
